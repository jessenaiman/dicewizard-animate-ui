---
title: Tooltip
description: A popup that displays information related to an element when the element receives keyboard focus or the mouse hovers over it.
author:
  name: Skyleen77
  url: https://github.com/Skyleen77
---

We take the [Shadcn UI](https://ui.shadcn.com/docs/components/tooltip) component and animate it with Motion.

<ComponentPreview name="radix-tooltip-demo" />

## Installation

<ComponentInstallation name="radix-tooltip" />

## Props

<div className="flex flex-row gap-x-3">
  <ExternalLink
    href="https://www.radix-ui.com/primitives/docs/components/tooltip"
    text="Docs"
  />
  <ExternalLink
    href="https://www.radix-ui.com/primitives/docs/components/tooltip#api-reference"
    text="API Reference"
  />
</div>

### Animate UI props

#### TooltipContent

<TypeTable
  type={{
    transition: {
      description: 'The transition of the tooltip content animation',
      type: 'Transition',
      required: false,
      default: "{ type: 'spring', stiffness: 300, damping: 25 }",
    },
  }}
/>

## Credits

- We use [Radix UI](https://www.radix-ui.com/primitives/docs/components/tooltip) for the tooltip component.
- We take our inspiration from [Shadcn UI](https://ui.shadcn.com/docs/components/tooltip) for the tooltip style (to make it easier to replace a shadcn component with the animate ui component).
