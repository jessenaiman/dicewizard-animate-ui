---
title: Dropdown Menu
description: Displays a menu to the user — such as a set of actions or functions — triggered by a button.
author:
  name: Skyleen77
  url: https://github.com/Skyleen77
new: true
---

We take the [Shadcn UI](https://ui.shadcn.com/docs/components/dropdown-menu) component and animate it with Motion.

<ComponentPreview name="radix-dropdown-menu-demo" />

## Installation

<ComponentInstallation name="radix-dropdown-menu" />

## Examples

### Checkboxes

<ComponentPreview name="radix-dropdown-menu-checkboxes-demo" />

### Radio Group

<ComponentPreview name="radix-dropdown-menu-radio-group-demo" />

## Props

<div className="flex flex-row gap-x-3">
  <ExternalLink
    href="https://www.radix-ui.com/primitives/docs/components/dropdown-menu"
    text="Docs"
  />
  <ExternalLink
    href="https://www.radix-ui.com/primitives/docs/components/dropdown-menu#api-reference"
    text="API Reference"
  />
</div>

### Animate UI props

#### DropdownMenuSubTrigger

<TypeTable
  type={{
    inset: {
      description: 'Whether the dropdown menu sub trigger is inset',
      type: 'boolean',
      required: false,
      default: 'false',
    },
    transition: {
      description:
        'The transition of the dropdown menu sub trigger hover animation',
      type: 'Transition',
      required: false,
      default: "{ type: 'spring', stiffness: 200, damping: 20 }",
    },
  }}
/>

#### DropdownMenuContent

<TypeTable
  type={{
    transition: {
      description: 'The transition of the dropdown menu content animation',
      type: 'Transition',
      required: false,
      default: '{ duration: 0.2 }',
    },
  }}
/>

#### DropdownMenuItem

<TypeTable
  type={{
    inset: {
      description: 'Whether the dropdown menu item is inset',
      type: 'boolean',
      required: false,
      default: 'false',
    },
    transition: {
      description: 'The transition of the dropdown menu item hover animation',
      type: 'Transition',
      required: false,
      default: "{ type: 'spring', stiffness: 200, damping: 20 }",
    },
  }}
/>

#### DropdownMenuLabel

<TypeTable
  type={{
    inset: {
      description: 'Whether the dropdown menu label is inset',
      type: 'boolean',
      required: false,
      default: 'false',
    },
  }}
/>

## Credits

- We use [Radix UI](https://www.radix-ui.com/primitives/docs/components/dropdown-menu) for the dropdown menu component.
- We take our inspiration from [Shadcn UI](https://ui.shadcn.com/docs/components/dropdown-menu) for the dropdown menu style (to make it easier to replace a shadcn component with the animate ui component).
