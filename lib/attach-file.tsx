  ...
  if ('_openapi' in data && typeof data._openapi === 'object') {
    ...
    if (meta.new) {
      node.name = (
        <>
          {node.name}{' '}
          <div className="ms-auto text-[10px] text-nowrap bg-blue-500 text-white rounded-sm leading-1 px-1 py-0.5 h-4.5 font-semibold flex items-center justify-center">
            <span>NEW</span>
          </div>
        </>
      );
